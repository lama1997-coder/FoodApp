...
const Home = () => {
 ...
 function getCategoryNameById(categoryId){
  ...
  function renderResturanList() {
    const renderResturanList = ({item}) => (
      <TouchableOpacity
        style={{
          ...
          <Image
                 ...
                 }
           {
             ...
             [1,2,3].map((priceRating)=>(
              ...
              style={{...FONTS.body3,color:(priceRating<=item.priceRating)?
  ...
  function renderHeader() {
    return (
      <View style={{flexDirection: 'row', height: 50, color: COLORS.black}}>
        <TouchableOpacity
          style={{
            width: 50,
  ...
  function onSelectCategory(category) {
    let resturantList = restaurantData.filter(x =>
      x.categories.includes(category.id),
    );
