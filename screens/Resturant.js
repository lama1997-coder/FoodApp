...
const Restaurant = ({route, navigation}) => {
  ...
  const [resturant, setRestaurant] = React.useState(null);
  const [currentLocation, setCurrentLocation] = React.useState(null);

  const [orderItem, setOrderItems] = React.useState([]);
  ...
  function getOrderQty(menuId){
    ...
    let qyt = orderItem.filter(a=>a.menuId==menuId)
          ...
          <View key={`menu-${index}`} style={{alignItems: 'center'}}>
            <View style={{height: SIZES.height * 0.35}}>
              <Image
                source={item.photo}
                resizeMode="cover"
                style={{width: SIZES.width, height: '100%'}}></Image>
